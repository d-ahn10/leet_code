...
var majorityElement = function(nums) {
    ...
    // create a sorted nums and save it into a unit called sortedNums
    ...
    // step through each number in the sortedNums and save each item into a unit called eachNum
    ...
    for(let i = 0; i < sortedNums.length; i++) {
        let eachNum = sortedNums[i];
        createTable(eachNum);
    }
    
    for(let key in tableNumObj) {
        if(tableNumObj[key] > sortedNums.length / 3) {
            ansNumArr.push(key);
        }
    }
